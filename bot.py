...
APP_URL = (os.getenv("APP_URL", "") or os.getenv("RENDER_EXTERNAL_URL", "")).strip()  # manual or Render auto URL
# Railway public domain fallback will be computed at runtime (see __main__).
...
if __name__ == "__main__":
    ...
    # ---- AUTO APP_URL (Render + Railway) ----
    # Priority:
    # 1) APP_URL (manual)
    # 2) RENDER_EXTERNAL_URL (Render Web)
    # 3) RAILWAY_PUBLIC_DOMAIN (Railway Public Networking)
    # 4) RAILWAY_STATIC_URL (Railway - if present)
    ...
    if not app_url:
        ...
        if railway_domain:
            # Usually like: myapp-production.up.railway.app (no scheme)
            app_url = f"https://{railway_domain}"

    if not app_url:
        railway_static = (os.getenv("RAILWAY_STATIC_URL") or "").strip()
        if railway_static:
            # Sometimes includes scheme, sometimes not
            app_url = railway_static if railway_static.startswith("http") else f"https://{railway_static}"

    # ---- RUN ----
    if app_url:
        # Railway provides PORT for web services; 8080 is a safe fallback.
